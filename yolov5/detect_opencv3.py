                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in det:
                        if save_crop:  # Save cropped prediction boxes
                            save_path = increment_path(save_dir / Path(p).stem, mkdir=True)  # increment run
                            save_crop_path = save_path / f"{names[int(cls)]}.jpg"
                            cv2.imwrite(str(save_crop_path), im0[int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])])
                        ...
                        # Write to CSV
                        if save_csv:
                            write_to_csv(Path(p).stem, names[int(cls)], conf.item())
