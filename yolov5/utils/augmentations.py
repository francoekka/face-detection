...
def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    # Scale ratio (new / old)
    r = min(new_shape[0] / shape[0], new_shape[1] / shape[1])
    #print(f"Scale ratio: {r}")

    if not scaleup:  # only scale down, do not scale up (for better val mAP)
        r = min(r, 1.0)
    
    #print(f"Adjusted scale ratio: {r}")

    # Compute padding
    ratio = r, r  # width, height ratios
    new_unpad = int(round(shape[1] * r)), int(round(shape[0] * r))
    #print(f"New unpadded shape: {new_unpad}")
    
    dw, dh = new_shape[1] - new_unpad[0], new_shape[0] - new_unpad[1]  # wh padding
    #print(f"Padding before adjustments - dw: {dw}, dh: {dh}")
    ...
    #print(f"Padding after adjustments - dw: {dw}, dh: {dh}")

    dw /= 2  # divide padding into 2 sides
    dh /= 2

    #print(f"Final padding - dw: {dw}, dh: {dh}")

    if shape[::-1] != new_unpad:  # resize
        im = cv2.resize(im, new_unpad, interpolation=cv2.INTER_LINEAR)
    top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1))
    left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
    #print(f"Border - top: {top}, bottom: {bottom}, left: {left}, right: {right}")
