...
def run(
    ...
    #source=ROOT / "data/video.mp4",  # comment out file input
    ...
    if webcam:
        ...
        # dataset = LoadImages(source, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)  # comment out file input
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    confidence_str = f"{confidence:.2f}" if not hide_conf else ""
                    annotator.box_label(xyxy, f"{label} {confidence_str}", color=colors(c, True))

                    # Write to CSV if save_csv flag is enabled
                    if save_csv:
                ...
                if dataset.mode == "image":
                    ...
                    if vid_path[i] != save_path:  # new video
                        ...
                        vid_writer[i] = cv2.VideoWriter(
                            save_path,
                            cv2.VideoWriter_fourcc(*"mp4v"),  # H264 codec
                            fps,
                            (w, h),
                            isColor=True,
                        )
